...
/* ─── Constants ───────────────────────────────────────────── */
const SPORTS: { value: Sport; label: string; emoji: string; active: boolean }[] = [
  { value: "padel", label: "Padel", emoji: "🎾", active: true },
  { value: "tennis", label: "Tennis", emoji: "🎾", active: false },
  { value: "pickleball", label: "Pickleball", emoji: "🏓", active: false },
];
...
function matchesTime(start: string, filter: TimeFilter): boolean {
  const m = parseMinutes(start);
  if (filter === "all") return true;
  if (filter === "morning") return m >= 360 && m < 720;
  if (filter === "afternoon") return m >= 720 && m < 1080;
  if (filter === "evening") return m >= 1080;
  return true;
}
...
function CourtLines() {
  return (
    <svg
      ...
      style={{ opacity: 0.11 }}
    >
      <rect x="24" y="18" width="272" height="178" stroke="white" strokeWidth="2" />
      <line x1="24" y1="107" x2="296" y2="107" stroke="white" strokeWidth="1.5" />
      <line x1="160" y1="18" x2="160" y2="196" stroke="white" strokeWidth="2" />
      <line x1="68" y1="18" x2="68" y2="196" stroke="white" strokeWidth="1" strokeDasharray="5 4" />
      <line x1="252" y1="18" x2="252" y2="196" stroke="white" strokeWidth="1" strokeDasharray="5 4" />
      <ellipse cx="160" cy="107" rx="28" ry="28" stroke="white" strokeWidth="1" />
    </svg>
  );
}

/* ─── Main component ──────────────────────────────────────── */
export default function PadelCourtsClient({
  clubs,
  slots,
  initialDate,
}: {
  ...
  const filteredClubs = useMemo(() => {
    ...
    return clubs.filter((club) => {
      if (q && !club.name.toLowerCase().includes(q) && !club.address.toLowerCase().includes(q))
        return false;
      // must have at least one slot matching selected dates + time
      return club.courts.some((court) =>
        (slotsByCourtId[court.id] ?? []).some(
          (s) => selectedDates.includes(s.date) && matchesTime(s.start_time, timeFilter),
        ),
      );
    });
  }, [clubs, search, selectedDates, timeFilter, slotsByCourtId]);

  /* Slots for a club grouped by date */
  function getSlotsForClub(club: Club): { date: string; slots: Slot[] }[] {
    ...
    return selectedDates
      ...
      .map((d) => ({ date: d, slots: grouped[d].sort((a, b) => a.start_time.localeCompare(b.start_time)) }));
  }
        ...
        <div className="max-w-5xl mx-auto px-5 py-4 flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm text-white placeholder-white/25 outline-none transition-all duration-200"
              style={{
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
              {SPORTS.map((s) => (
                <button
                  key={s.value}
                  onClick={() => s.active && setSport(s.value)}
                  suppressHydrationWarning
                  className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200"
                  style={{
                    fontFamily: "var(--font-syne, Syne, sans-serif)",
                    background: sport === s.value ? "#c9ff3b" : "transparent",
                    color:
                      sport === s.value
                        ? "#000"
                        : s.active
              ...
              {dateOptions.map((opt) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      fontFamily: "var(--font-syne, Syne, sans-serif)",
                      background: selected ? "rgba(201,255,59,0.15)" : "rgba(255,255,255,0.04)",
                      border: selected
                        ? "1px solid rgba(201,255,59,0.4)"
                        : "1px solid rgba(255,255,255,0.07)",
                      color: selected ? "#c9ff3b" : "rgba(255,255,255,0.5)",
                    }}
                  >
                    {opt.label}
                  </button>
            ...
            <div ref={timeMenuRef} className="relative ml-auto shrink-0">
              <button
                ...
                style={{
                  fontFamily: "var(--font-syne, Syne, sans-serif)",
                  background: timeFilter !== "all" ? "rgba(201,255,59,0.12)" : "rgba(255,255,255,0.04)",
                  border: timeFilter !== "all" ? "1px solid rgba(201,255,59,0.35)" : "1px solid rgba(255,255,255,0.07)",
                  color: timeFilter !== "all" ? "#c9ff3b" : "rgba(255,255,255,0.55)",
                }}
              >
                  ...
                  >
                    {TIME_OPTIONS.map((opt) => (
                      <button
                        ...
                        suppressHydrationWarning
                        className="w-full flex items-center justify-between px-4 py-2.5 text-left transition-all duration-150 hover:bg-white/[0.04] mx-0"
                        style={{ fontFamily: "var(--font-outfit, Outfit, sans-serif)" }}
                      >
                        <span
                          className="text-sm font-semibold"
                          style={{
                            color: timeFilter === opt.value ? "#c9ff3b" : "rgba(255,255,255,0.8)",
                            fontFamily: "var(--font-syne, Syne, sans-serif)",
                          }}
                        >
                          {opt.label}
          ...
          >
            {filteredClubs.map((club, i) => (
              <ClubCard
                ...
                slotGroups={getSlotsForClub(club)}
                isFav={favorites.has(club.id)}
                onToggleFav={() =>
                  setFavorites((prev) => {
                    const next = new Set(prev);
                    next.has(club.id) ? next.delete(club.id) : next.add(club.id);
                    return next;
                  })
                }
...
function ClubCard({
  ...
  club: Club;
  gradient: string;
  slotGroups: { date: string; slots: Slot[] }[];
  isFav: boolean;
  onToggleFav: () => void;
  multiDay: boolean;
}) {
  ...
  const minPrice = club.courts.length
    ? Math.min(...club.courts.map((c) => c.price_per_hour))
    : null;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ flex: "0 0 calc(100% * 2 / 3)", background: gradient }}
      >
        <CourtLines />
        ...
        >
          <span
            ...
            style={{ color: isFav ? "#ff3b5c" : "rgba(255,255,255,0.5)" }}
          >
            {isFav ? "♥" : "♡"}
          </span>
        </button>
      ...
      {/* Content – 1/3 height */}
      <div
        className="flex flex-col px-4 pt-3 pb-3 gap-2 overflow-hidden"
        style={{ flex: "0 0 calc(100% / 3)" }}
      >
        {/* Location */}
        <p className="text-xs text-white/40 truncate shrink-0">
          📍 {club.address}, Tbilisi
        </p>
        ...
        <div className="flex flex-col gap-1.5 overflow-hidden flex-1">
          {slotGroups.slice(0, multiDay ? 3 : 1).map(({ date, slots }) => (
            <div key={date} className="flex items-center gap-2 min-w-0">
              {multiDay && (
                <span
                  className="text-[10px] font-bold text-white/30 shrink-0 w-12"
                  style={{ fontFamily: "var(--font-syne, Syne, sans-serif)" }}
                >
                  {fmtDate(date)}
                </span>
              ...
              <div className="flex gap-1.5 overflow-x-auto scrollbar-hide flex-nowrap">
                {slots.map((slot) => (
                  <a
                    ...
                    className="shrink-0 text-[11px] font-semibold px-2.5 py-1 rounded-lg transition-all duration-150 hover:bg-[#c9ff3b] hover:text-black"
                    style={{
